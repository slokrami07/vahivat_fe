    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold tracking-tight font-fraunces text-ink">Discovery Hub</h1>
          <p className="text-charcoal">Find the perfect vendors, hardware, or SaaS platforms for your enterprise.</p>
        </div>

        <Tabs value="talent" onValueChange={() => {}} className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-8">
            <TabsTrigger value="talent">IT Talent & Agencies</TabsTrigger>
            <TabsTrigger value="hardware">Hardware Bulk</TabsTrigger>
            <TabsTrigger value="saas">SaaS Licenses</TabsTrigger>
          </TabsList>
          
          <TabsContent value="talent" className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-ink font-fraunces">Top Rated Agencies</h2>
              <Button variant="outline" size="sm" className="gap-2 text-charcoal hover:text-ink"><Filter className="h-4 w-4" /> Filter</Button>
            </div>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {[
                { name: "TechNova Agency", size: "50-200", tech: ["React", "Node.js", "AWS"], rating: "4.9", projects: 124 },
                { name: "CodeCraft Studios", size: "10-50", tech: ["Vue", "Python", "GCP"], rating: "4.8", projects: 89 },
                { name: "DataFlow Partners", size: "200+", tech: ["Java", "Angular", "Azure"], rating: "4.7", projects: 412 },
              ].map((agency, i) => (
                <Card key={i} className="flex flex-col overflow-hidden hover:border-terracotta/50 group cursor-pointer bg-white">
                  ...
                  <CardHeader className="-mt-8 pb-4">
                    ...
                    <CardDescription className="text-charcoal/80">Team Size: {agency.size}</CardDescription>
                  </CardHeader>
                  <CardContent className="flex-1 space-y-4">
                    <div className="flex flex-wrap gap-2">
                      {agency.tech.map(t => <Badge key={t} variant="secondary" className="bg-cream text-charcoal hover:bg-cream border-none">{t}</Badge>)}
                    </div>
                  ...
                  <CardFooter>
                    <Button className="w-full bg-terracotta hover:bg-terracotta/90 text-white transition-all rounded-full">Request Profile</Button>
                  </CardFooter>
                </Card>
              ))}
            </div>
  ...
  return (
    <div className="relative">
      <div className={cn("space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500", isDrawerOpen && "pr-[400px] lg:pr-[500px] transition-all duration-300 ease-in-out")}>
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold tracking-tight font-fraunces text-ink">Scan Opportunities</h1>
          <p className="text-charcoal">Discover and bid on anonymized RFPs matching your capabilities.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
          <div className="flex flex-wrap gap-2">
            {["All", "Software Dev", "Hardware Pool", "SaaS Platforms"].map(filter => (
              <Badge 
                key={filter} 
        ...
        <div className="grid gap-6">
          {filteredOpportunities.length > 0 ? filteredOpportunities.map((rfp) => (
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-charcoal/80 flex items-center gap-1">
                      <ClockIcon className="h-3 w-3" /> Published {rfp.deadline}
                    </span>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {rfp.tags.map(t => <Badge key={t} variant="outline" className="bg-white text-charcoal border-slate-200">{t}</Badge>)}
                    </div>
                  </div>
                  <div className="text-right whitespace-nowrap">
                    <div className="text-sm text-charcoal mb-1">Budget Range</div>
            ...
            <div className="text-center py-12 text-charcoal border-2 border-dashed border-slate-200 rounded-xl bg-white">
        ...
        {selectedOpportunity && (
          <>
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h2 className="text-xl font-bold font-fraunces text-ink">Submit Proposal</h2>
            ...
            <div className="p-6 border-b border-slate-100 bg-cream/50">
              <div className="text-sm text-charcoal mb-1">Bidding on</div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <form id="proposal-form" onSubmit={submitProposal} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-ink">Estimated Pricing ($)</label>
                  ...
                  <p className="text-xs text-charcoal/70">Provide your best estimate based on the scope.</p>
                </div>
                
                <div className="space-y-2">
                  <label className="text-sm font-medium text-ink">Timeline Estimate</label>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-ink">Pitch / Cover Letter</label>
                  <textarea 
                    required
                    ...
                    placeholder="Why is your team the best fit for this project? Highlight relevant experience..."
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-sm font-medium text-ink">Attachments (Optional)</label>
                  <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-cream transition-colors">
                    ...
                    <span className="text-sm font-medium text-ink">Attach Portfolio or Case Studies</span>
                    <span className="text-xs text-charcoal/70">PDF up to 10MB</span>
                  </div>
                </div>
              </form>
            </div>
